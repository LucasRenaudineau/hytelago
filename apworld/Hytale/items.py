...
FILLER_ITEM_NAME = "Ingredient_Poop"

# Item-id ranges (mirrors ITEM_NAME_TO_ID):
#   1000-1999  monster-spawn traps
#   2000-2999  tier upgraders
#   3000-3999  random loot (incl. the filler Ingredient_Poop)
TRAP_ITEMS: list[str] = [n for n, i in ITEM_NAME_TO_ID.items() if 1000 <= i <= 1999]
TIER_ITEMS: list[str] = [n for n, i in ITEM_NAME_TO_ID.items() if 2000 <= i <= 2999]
# Random loot, excluding the filler item which is added separately.
LOOT_ITEMS: list[str] = [
    n for n, i in ITEM_NAME_TO_ID.items() if 3000 <= i <= 3999 and n != FILLER_ITEM_NAME
]

# Real items always fill up to this fraction of the locations; the rest is filler.
REAL_ITEM_FILL_RATIO = 0.95
...
def create_all_items(world: HytaleWorld) -> None:
    ...
    # --- Guaranteed base ---------------------------------------------------
    # All tier upgraders at their maximum quantity.
    counts: dict[str, int] = {name: ITEM_POOL_QUANTITIES[name] for name in TIER_ITEMS}
    # All random loot once (Teleporter3 always starts at 2).
    for name in LOOT_ITEMS:
        counts[name] = 1
    counts["Teleporter3"] = 2
    # Death Link seeds a single copy of every trap.
    if options.death_link:
        for name in TRAP_ITEMS:
            counts[name] = 1

    # --- Fill in id order up to each item's max, until 95% of locations -----
    target_real = int(num_locations * REAL_ITEM_FILL_RATIO)
    for name, max_qty in ITEM_POOL_QUANTITIES.items():
        if sum(counts.values()) >= target_real:
            break
        # Traps only grow past their seeded copy when the Traps option is on.
        if name in TRAP_ITEMS and not options.traps:
            continue
        while counts.get(name, 0) < max_qty and sum(counts.values()) < target_real:
    ...
    # Remaining slots become filler.
    needed_filler = num_locations - len(itempool)
