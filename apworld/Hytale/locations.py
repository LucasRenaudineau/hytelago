...
#
# LOCATION_NAME_TO_ID above is the full catalogue of every location the game
# can ever offer. Which of those actually exist in a given seed depends on the
# player's yaml options, so the table is built fresh from the options below.
#
# Location-id ranges:
...
def get_active_location_names(world: HytaleWorld) -> list[str]:
    options = world.options
    active: list[str] = []

    for name, loc_id in LOCATION_NAME_TO_ID.items():
        if 3000 <= loc_id <= 3999:
            if not options.death_locations:
                continue
        elif 5000 < loc_id <= 5999:
            if not options.memories:
                continue
            index = loc_id - 5000
            if index > options.memories_max.value:
                continue
            # memories_every == 2 keeps memories_1, memories_3, memories_5, ...
            if (index - 1) % options.memories_every.value != 0:
                continue
        elif 6000 <= loc_id <= 6999:
